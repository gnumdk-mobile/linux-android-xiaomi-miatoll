...
static void update_vm_policy(bool power_save)
{
	if (power_save) {
...
static void update_devfreq_policy(bool power_save)
{
	struct devfreq_device *device;
	int i, j;
	...
	for (i = 0; i < DEVFREQ_MAX; i++) {
		...
		for (j = 0; j < device->count ; j++) {
			...
			if (power_save) {
				device->devfreq[j]->max_freq = devfreq_frequencies[device->type];
			} else {
...
static int update_thread(void *data)
{
	...
	while (1) {
		...
		streams = _streams;
		power_save = !screen_on && streams < 1;

		update_online_cpu_policy();
		update_devfreq_policy(power_save);
		update_vm_policy(power_save);
	}
			   ...
			   void *data)
{
	...
	streams = atomic_read(&power_saver_drv.streams);

	if (power_saver_drv.screen_on || streams > 0) {
			       ...
			       void *data)
{
	...
	if (*blank == MSM_DRM_BLANK_UNBLANK) {
		power_saver_drv.screen_on = true;
	} else if (*blank == MSM_DRM_BLANK_POWERDOWN) {
		power_saver_drv.screen_on = false;
	}
